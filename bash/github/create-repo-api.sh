...
#
#   ./create-repo-api.sh <RepoName> [--org <Org>] [--public]
set -euo pipefail
...
if [ -z "$REPO_NAME" ]; then
  echo "Nutzung: ./create-repo-api.sh <RepoName> [--org <Org>] [--public]" >&2
  exit 1
fi

bw_new_template_repo "$REPO_NAME" "bieberworks-api" "docker" "$ORG" "$VIS"
